  ...
  <div>
    ...
    <v-card class="mx-auto" max-width="750">
      <v-card-title class="white--text blue">
        ...
        <template>
          <v-btn color="white" class="text--primary" dark to="/register">
        ...
        <template v-slot:default="{ item }">
          <v-list-item>
            <v-list-item-content>
              ...
              <v-list-item-title
                >Department:{{ item.user_name }}</v-list-item-title
              >
            </v-list-item-content>
  ...
  methods: {
    ...
    handleRenamingUser() {
      if (this.$refs.form.validate()) {
        ...
        this.$http.get("/sanctum/csrf-cookie").then((res) => {
          this.$http
            .put("/api/departments/" + this.user_id_to_rename, {
              name: this.user_rename_name,
              device_name: "browser",
            })
            .then((response) => {
              console.log(res);
